...
- Forest: 32, multipolygon
- Parks: 47, mp
- Playgrounds: 51, point
- Shelters: 19, p

2. What is the CRS value in these objects?
- Forest: Projected CRS: ETRS89 / UTM zone 32N
- Parks: Projected CRS: ETRS89 / UTM zone 32N
- Playgrounds: NA
- Shelters: Projected CRS: ETRS89 / UTM zone 32N
...
# Load the sf package and the dplyr package
pacman::p_load(dplyr,sf)

# Use filter() to limit to over5ha parks
over5ha <- filter(parks,areal_m2 > 50000)
...
as_tibble()# Pull out the third observation from both columns individually
df$a[3]
df$b[3]
```
...
plotRGB(aarhus, stretch = "lin"); plot(elevation, add = T)
...
st_crs(aarhus) # +proj=utm +zone=32 +ellps=GRS80 +units=m +no_defs 
st_crs(elevation) # +proj=longlat +ellps=WGS84 +no_defs 
